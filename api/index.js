...
async function fetchUSFearGreed() {
  try {
    ...
    return {
      score: s, label: getLabelKR(s), rating: fg.rating,
      previous_close: Math.round(fg.previous_close || s),
      previous_1_week: Math.round(fg.previous_1_week || s),
      indicators: [
        { name: '시장 모멘텀', value: s }, { name: '변동성 (VIX)', value: Math.round(100 - s * 0.3) },
...
async function fetchUSFallback() {
  try {
    ...
    return {
      score, label: getLabelKR(score), vix: vix.price, sp500_change: sp500.changePercent.toFixed(2),
      indicators: [
        { name: '시장 모멘텀', value: momentumScore }, { name: '변동성 (VIX)', value: vixScore },
    ...
    return { score: 45, label: '중립', indicators: Array(7).fill({ name: '-', value: 45 }), source: '로딩 실패' };
  }
}
...
async function fetchKRFearGreed() {
  try {
    ...
    const indicators = [
      { name: '시장 모멘텀', value: Math.round(momentum) }, { name: '주가 강도', value: Math.round(strength) },
    ...
    return {
      score, label: getLabelKR(score),
      kospi_price: kospi.price.toFixed(2), kospi_change: kospi.changePercent.toFixed(2),
      kosdaq_change: kosdaq.changePercent.toFixed(2), vkospi: vkospi.price.toFixed(2),
      indicators, source: 'Yahoo Finance KOSPI'
    };
  } catch (e) {
    return { score: 40, label: '공포', indicators: Array(7).fill({ name: '-', value: 40 }), source: '로딩 실패' };
  }
}
